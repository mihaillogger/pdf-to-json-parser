...
Важно различать **две независимые вещи**:

* **Интернет** — это только CrossRef (обогащение метаданных по DOI). Рубится флагом `--offline`.
* **Локальные нейросети** — `qwen2.5:3b`, `llava`, YOLO, Pix2Tex. Работают на вашей машине,
  интернет им не нужен; включены по умолчанию. Их отключают **ради скорости**, а не из-за сети.

Что конкретно выключает каждый флаг:

| Флаг | Что отключает | Эффект |
| --- | --- | --- |
| `--offline` | обращения к CrossRef (интернет) | работаем без сети; локальные модели остаются |
| `--no-crossref` | только CrossRef | то же, но интернет для прочего разрешён |
| `--no-llm` | LLM-добор метаданных (`qwen2.5:3b`) | метаданные только эвристикой — быстрее, но менее точно |
| `--no-extract-images` | детекцию фигур и таблиц (DocLayout-YOLO + `llava`) | **снимает главный тормоз** — батч летает |

Готовые комбинации:

| Режим | Команда | Что использует |
| --- | --- | --- |
| **Онлайн (макс. качество)** | *(по умолчанию)* | CrossRef + LLM + YOLO/VLM (фигуры, таблицы, формулы) |
| **Офлайн + LLM** | `--offline` | без интернета; локальная LLM и нейросети работают |
| **Офлайн без LLM** | `--offline --no-llm` | без сети и без LLM, только эвристики метаданных |
| **Быстрый батч** | `--offline --no-llm --no-extract-images` | максимально быстро: текст, метаданные, секции (без тяжёлого YOLO/VLM) |
