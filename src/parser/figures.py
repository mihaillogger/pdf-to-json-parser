...
from parser.schemas import Table, Figure, BBox, Panel, TableDataResponse
...
class VLMTableExtractor:
    ...
    def extract_2d_array(self, image_path: str) -> List[List[str]]:
        ...
        prompt = (
            ...
            "Rules:\n"
            "1. Output ONLY valid JSON containing a single key 'data' with a value of type list[list[string]].\n"
            "2. No markdown formatting, no explanations.\n"
            "3. Handle merged header cells by duplicating the text or keeping structure intact."
        )
...
class SpatialExtractor:
    ...
    def extract_visuals(self, pdf_path: str) -> Tuple[List[Figure], List[Table]]:
        ...
        for page_num in range(len(doc)):
            page = doc[page_num]
            temp_img_path = os.path.join(self.output_img_dir, f"temp_yolo_p{page_num}.png")
            page.get_pixmap(dpi=72).save(temp_img_path)
            
            results = self.model.predict(temp_img_path, imgsz=1024, conf=0.25, verbose=False)
            ...
            for box in boxes:
                ...
                bbox_obj = BBox(left=round(x1, 2), top=round(y1, 2), right=round(x2, 2), bottom=round(y2, 2))
