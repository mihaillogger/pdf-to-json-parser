...
class Metadata(BaseSchema):
    ...
    abstract: str | None = Field(..., description="Аннотация статьи (обязательное поле, допускает null)")
    keywords: list[str] = Field(default_factory=list, description="Ключевые слова")
    doi: str | None = Field(..., description="DOI документа (обязательное поле, допускает null)")
...
class Document(BaseSchema):
    ...
    acknowledgments: str | None = Field(None, description="Текст раздела Acknowledgements или null")
