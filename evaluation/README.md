...
**online (CrossRef):** все поля 1.000 — это и эталон, и верхняя граница качества.

Скаляры сравниваются с нормализацией (регистр/диакритика/пунктуация); `title`/`journal`
допускают вложенность строк, `abstract` — похожесть текста ≥ `TEXT_RATIO_THRESHOLD`
(длинный текст ломается от одного символа при строгом substring), `doi`/`year` — строго.
